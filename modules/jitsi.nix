...
let
  ...
  sipProps = pkgs.writeText "sip-communicator.properties" ''
...
in
{
  ...
  config = mkIf cfg.enable {
    ...
    networking.firewall = {
      allowedTCPPorts = [ 80 443 5349 ];   # TURN-TLS
      allowedUDPPorts = [ 10000 3478 ];    # media + TURN-UDP
    };

    environment.etc."jitsi/videobridge/sip-communicator.properties".source = sipProps;

    systemd.services.jitsi-videobridge2 = {
      wants = [ "network-online.target" ];
      after = [ "network-online.target" ];
      restartTriggers = [ sipProps ];
    };
  };
}